...
public class BaxterController : MonoBehaviour
{
    ...
    private string goalReachedTopicName = "baxter_target_reached";
    ...
    public void TrajectoryResponse(BaxterTrajectory response)
    {
        if (response.trajectory.Length > 0)
        {
            ...
            coroutine = ExecuteTrajectories(response);
    ...
    public void GripperResponse(Bool response)
    {
        if (response.data)
        {
	    CloseGripper("left");
    ...
    public void RestResponse(RosMessageTypes.Std.String response)
    {
        GoToRestPosition(response.data);
    }

    public void StopResponse(RosMessageTypes.Std.Bool response)
    {
        if(response.data)
    ...
    public PoseStamped GetRightGripper(){
        
        var now = DateTime.Now;
        var header = new Header(seq: (uint)1, stamp: new Time((uint)now.Second, 0), frame_id: "gripper_r");
        var rightGripper = new PoseStamped{
            header = header,
            pose = new RosMessageTypes.Geometry.Pose{
                position = (gripperBaseR.position).To<FLU>(),
                orientation = (gripperBaseR.rotation).To<FLU>()
            }
        };
    ...
    public PoseStamped GetLeftGripper(){
        
        var now = DateTime.Now;
        var header = new Header(seq: (uint)1, stamp: new Time((uint)now.Second, 0), frame_id: "gripper_l");
        var leftGripper = new PoseStamped{
            header = header,
            pose = new RosMessageTypes.Geometry.Pose{
                position = (gripperBaseL.position).To<FLU>(),
                orientation = (gripperBaseL.rotation).To<FLU>()
            }
        };
    ...
    private IEnumerator ExecuteTrajectories(BaxterTrajectory response)
    {
        if (response.trajectory != null)
        {
    	    ...
    	    var target = new Bool();
    	    target.data = true;
    	    ros.Send(goalReachedTopicName, target);
            //yield return new WaitForSeconds(poseAssignmentWait);
